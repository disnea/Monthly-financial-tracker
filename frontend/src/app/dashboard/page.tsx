...
import { 
  ...
  ArrowUp, ArrowDown, Minus, Info, CalendarClock, Receipt, ShieldCheck, Zap
} from 'lucide-react'
...
// ── Pill-based date range selector ──────────────────────────────────
...
function PillDateSelector({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      try {
        ...
        // If API returned too few points for a meaningful chart, pad with synthetic history
        if (netWorthData.length < 3) {
          const baseWorth = netWorthData[0]?.net_worth ?? 0
          const now = new Date()
          const padded: any[] = []
          for (let i = 11; i >= 0; i--) {
            const date = new Date(now.getFullYear(), now.getMonth() - i, 1)
            const drift = (Math.random() - 0.4) * baseWorth * 0.03
            padded.push({
              date: date.toISOString().split('T')[0],
              net_worth: Math.round(baseWorth * (0.92 + (11 - i) * 0.008) + drift),
              change_percent: 0,
              health_score: healthData?.overall_score ?? 0,
            })
          }
          // replace last entry with real current value
          if (netWorthData.length > 0) {
            padded[padded.length - 1] = { ...padded[padded.length - 1], ...netWorthData[netWorthData.length - 1] }
          }
          // recalculate change_percent
          for (let i = 1; i < padded.length; i++) {
            const prev = padded[i - 1].net_worth
            padded[i].change_percent = prev > 0 ? ((padded[i].net_worth - prev) / prev) * 100 : 0
          }
          netWorthData = padded
          setIsMockData(true)
        } else {
    ...
    <div className="space-y-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <PillDateSelector value={dateRange} onChange={setDateRange} />
                </div>
                ...
                <div className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-800/50 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Net Worth</span>
                      {isMockData && (
                  ...
                  <div className="flex items-baseline gap-3">
                    ...
                    {stats.investmentReturn !== 0 && (
                      <div className={cn("flex items-center gap-1 px-2 py-1 rounded-lg", stats.investmentReturn >= 0 ? "text-emerald-600 bg-emerald-50" : "text-rose-600 bg-rose-50")}>
                        ...
                        <span className="text-sm font-semibold tabular-nums">{stats.investmentReturn >= 0 ? '+' : ''}{stats.investmentReturn.toFixed(1)}%</span>
                      </div>
                    )}
                  </div>
                  {showBalance && (
              ...
              <div className="grid grid-cols-2 gap-4 lg:w-[420px]">
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{format(budgetRemaining)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{format(stats.emiMonthly)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{budgetUtilization.toFixed(0)}%</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{format(stats.investmentValue)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{format(stats.thisMonthIncome)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{format(stats.borrowingsOwed)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm col-span-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white">{format(stats.lendingsOutstanding)}</p>
      ...
      {loading ? (
        ...
        >
          ...
          <Card className="relative border border-slate-200 dark:border-slate-800 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 rounded-2xl overflow-hidden group bg-white dark:bg-slate-900 h-full">
            <div className="absolute inset-0 bg-gradient-to-br from-rose-50 to-pink-50 opacity-30 dark:opacity-10"></div>
            <CardContent className="p-7 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className={cn("w-12 h-12 rounded-xl bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center shadow-lg group-hover:shadow-rose-500/25 transition-shadow")}>
              ...
              <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Total Expenses</h3>
              <p className="text-3xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white mb-2">{format(stats.totalExpenses)}</p>
          ...
          <Card className="relative border border-slate-200 dark:border-slate-800 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 rounded-2xl overflow-hidden group bg-white dark:bg-slate-900 h-full">
            ...
            <CardContent className="p-7 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg group-hover:shadow-emerald-500/25 transition-shadow">
              ...
              <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Total Income</h3>
              <p className="text-3xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white mb-2">{format(stats.totalIncome)}</p>
          ...
          <Card className="relative border border-slate-200 dark:border-slate-800 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 rounded-2xl overflow-hidden group bg-white dark:bg-slate-900 h-full">
            ...
            <CardContent className="p-7 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center shadow-lg group-hover:shadow-blue-500/25 transition-shadow">
              ...
              <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Savings Rate</h3>
              <p className="text-3xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white mb-2">{stats.savingsRate.toFixed(1)}%</p>
          ...
          <Card className="relative border border-slate-200 dark:border-slate-800 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 rounded-2xl overflow-hidden group bg-white dark:bg-slate-900 h-full">
            ...
            <CardContent className="p-7 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg group-hover:shadow-amber-500/25 transition-shadow">
              ...
              <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Budget Status</h3>
              <p className="text-3xl font-bold tracking-tight tabular-nums text-slate-900 dark:text-white mb-2">{budgetUtilization.toFixed(0)}%</p>
                    ...
                    <RechartsPie>
                      ...
                      <Tooltip content={({ active, payload }) => {
                        if (active && payload && payload[0] && payload[0].value) {
                          return (<div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg p-3"><p className="font-semibold text-sm dark:text-white">{payload[0].payload.name}</p><p className="text-lg font-bold tabular-nums text-slate-700 dark:text-slate-200">{format(payload[0].value as number)}</p></div>)
                        }
                        return null
                      }} />
                      {/* Center label */}
                      <text x="50%" y="46%" textAnchor="middle" dominantBaseline="central" className="fill-slate-400 text-[10px] font-semibold uppercase">Total</text>
                      <text x="50%" y="56%" textAnchor="middle" dominantBaseline="central" className="fill-slate-900 dark:fill-white text-base font-bold">{format(expensePieData.reduce((s, d) => s + d.value, 0))}</text>
                    </RechartsPie>
                  </ResponsiveContainer>
                ...
                <>
                  <ResponsiveContainer width="100%" height={240}>
                    <ComposedChart data={incomeVsExpenseData}>
                      ...
                      <Area type="monotone" dataKey="savings" fill="#3b82f6" fillOpacity={0.1} stroke="#3b82f6" strokeWidth={2} dot={{ r: 3, fill: '#3b82f6' }} />
                    </ComposedChart>
                  </ResponsiveContainer>
      ...
      {!loading && (
        <>
        {/* ── Safe-to-Spend + Financial Health Row ─────────────── */}
        <motion.div
        ...
        >
          {/* Safe-to-Spend */}
          <Card className="relative overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl rounded-3xl bg-white dark:bg-slate-900 flex flex-col">
            <div className="absolute -top-20 -right-20 w-56 h-56 bg-gradient-to-br from-indigo-500/10 to-purple-500/10 rounded-full blur-3xl" />
            <div className="absolute -bottom-16 -left-16 w-40 h-40 bg-gradient-to-tr from-emerald-500/5 to-transparent rounded-full blur-2xl" />
            <CardContent className="p-6 relative z-10 flex-1 flex flex-col">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Safe to Spend</h3>
                  <p className="text-xs text-slate-500">What you can still spend this month</p>
                </div>
              </div>
              {(() => {
                const now = new Date()
                ...
                const currentMonthIncome = allIncomes
                  .filter((i: any) => { const d = new Date(i.income_date); return d >= monthStart && d <= monthEnd })
                  .reduce((s: number, i: any) => s + i.amount, 0)
                const currentMonthExpenses = allExpenses
                  .filter((e: any) => { const d = new Date(e.transaction_date); return d >= monthStart && d <= monthEnd })
                  .reduce((s: number, e: any) => s + e.amount, 0)

                const totalObligations = currentMonthExpenses + stats.emiMonthly
                const safeToSpend = Math.max(0, currentMonthIncome - totalObligations)
                const daysInMonth = monthEnd.getDate()
                const daysLeft = daysInMonth - now.getDate()
                const daysPassed = now.getDate()
                const dailyBudget = daysLeft > 0 ? safeToSpend / daysLeft : 0
                const spentPct = currentMonthIncome > 0 ? Math.min((totalObligations / currentMonthIncome) * 100, 100) : 0
                ...
                const ringColor = spentPct >= 90 ? '#ef4444' : spentPct >= 70 ? '#f59e0b' : '#6366f1'
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex flex-col items-center mb-5">
                      <div className="relative" style={{ width: sz, height: sz }}>
                        <svg width={sz} height={sz} className="block -rotate-90">
                          <circle cx={sz / 2} cy={sz / 2} r={r} fill="none" strokeWidth={sw} className="stroke-slate-100 dark:stroke-slate-700/60" />
                          <motion.circle
                            cx={sz / 2} cy={sz / 2} r={r} fill="none"
                            stroke={ringColor} strokeWidth={sw} strokeLinecap="round"
                            strokeDasharray={circ}
                            initial={{ strokeDashoffset: circ }}
                            animate={{ strokeDashoffset: circ * (1 - spentPct / 100) }}
                            transition={{ duration: 1, delay: 0.2, ease: [0.32, 0.72, 0, 1] }}
                          />
                          {/* Time marker tick */}
                          {(() => {
                            const angle = (timePct / 100) * 360 - 90
                            const rad = (angle * Math.PI) / 180
                            const inner = r - sw / 2 - 2
                            const outer = r + sw / 2 + 2
                            return (
                              <line
                                x1={sz / 2 + Math.cos(rad) * inner} y1={sz / 2 + Math.sin(rad) * inner}
                                x2={sz / 2 + Math.cos(rad) * outer} y2={sz / 2 + Math.sin(rad) * outer}
                                className="stroke-slate-400 dark:stroke-slate-500" strokeWidth={2} strokeLinecap="round"
                              />
                            )
                          })()}
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-lg font-extrabold tabular-nums text-slate-900 dark:text-white leading-none">{spentPct.toFixed(0)}%</span>
                          <span className="text-[9px] font-semibold text-slate-400 mt-0.5">spent</span>
                        </div>
                      </div>
                      {/* Amount + status below ring */}
                      <p className="text-2xl font-extrabold tabular-nums text-slate-900 dark:text-white leading-tight mt-3">
                        {format(safeToSpend)}
                      </p>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs font-medium" style={{ color: ringColor }}>
                          {spentPct >= 90 ? 'Tight' : spentPct >= 70 ? 'Watch it' : 'On track'}
                        </span>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
                        <p className="text-[10px] font-semibold uppercase tracking-widest text-emerald-600 mb-1">Daily Budget</p>
                        <p className="text-lg font-bold tabular-nums text-emerald-700 dark:text-emerald-300">{format(dailyBudget)}</p>
                      </div>
                      <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                        <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 mb-1">Monthly EMI</p>
                        <p className="text-lg font-bold tabular-nums text-slate-900 dark:text-white">{format(stats.emiMonthly)}</p>
                      </div>
                    </div>

                    {/* Last 7 days spending mini bar chart */}
                    {(() => {
                      const bars: { day: string; amount: number }[] = []
                      for (let i = 6; i >= 0; i--) {
                        const d = new Date(now.getFullYear(), now.getMonth(), now.getDate() - i)
                        const dayTotal = allExpenses
                          .filter((e: any) => {
                            const ed = new Date(e.transaction_date)
                            return ed.getFullYear() === d.getFullYear() && ed.getMonth() === d.getMonth() && ed.getDate() === d.getDate()
                          })
                          .reduce((s: number, e: any) => s + e.amount, 0)
                        bars.push({ day: d.toLocaleDateString('en-US', { weekday: 'narrow' }), amount: dayTotal })
                      }
                      const maxBar = Math.max(...bars.map(b => b.amount), 1)
                      return (
                        <div className="mb-4">
                          <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-2">Last 7 Days</p>
                            ...
                            {bars.map((bar, i) => (
                              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                                <motion.div
                                  ...
                                  initial={{ height: 0 }}
                                  animate={{ height: `${Math.max((bar.amount / maxBar) * 48, 2)}px` }}
                                  transition={{ duration: 0.5, delay: 0.3 + i * 0.05 }}
                                />
                                <span className="text-[9px] text-slate-400 leading-none">{bar.day}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    {(() => {
                      ...
                      allExpenses
                        ...
                        .forEach((e: any) => {
                          ...
                          else catMap.set(id, { name: e.category_name ?? 'Other', color: e.category_color ?? '#64748b', total: e.amount })
                        })
                      const topCats = Array.from(catMap.values()).sort((a, b) => b.total - a.total).slice(0, 3)
                      if (topCats.length === 0) return null
                      const catMax = topCats[0]?.total || 1
                      return (
                        <div className="flex-1 mb-3">
                          <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-2">Top Categories</p>
                              ...
                              <div key={i}>
                                <div className="flex items-center justify-between mb-1">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs text-slate-600 dark:text-slate-400 truncate max-w-[100px]">{cat.name}</span>
                                  </div>
                                  <span className="text-xs font-semibold tabular-nums text-slate-700 dark:text-slate-300">{format(cat.total)}</span>
                                </div>
                    ...
                    <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-800 space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-semibold tabular-nums text-emerald-600">{format(currentMonthIncome)}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold tabular-nums text-rose-600">-{format(currentMonthExpenses)}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold tabular-nums text-slate-600 dark:text-slate-300">-{format(stats.emiMonthly)}</span>
                      </div>
                    </div>
                  </div>
                )
          ...
          {/* Financial Health Score - Standalone Card */}
          {healthScore && (
            <div className="lg:col-span-2">
              <HealthScoreIndicator healthData={healthScore} className="h-full" />
            </div>
          )}
        </motion.div>

        {/* ── Budget Progress Bars + Investment Allocation Row ── */}
        <motion.div
            ...
            <CardHeader className="border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-bold text-slate-900 dark:text-white">Budget Progress</CardTitle>
                    <CardDescription className="text-sm font-medium text-slate-600 dark:text-slate-400">Category breakdown</CardDescription>
                  </div>
                </div>
            ...
            <CardContent className="pt-4">
              {allBudgets.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 font-medium">No budgets set</p>
                  <p className="text-sm text-slate-500">Create budgets to track progress</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {allBudgets.slice(0, 6).map((budget: any, idx: number) => {
                      ...
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[140px]">
                              {budget.category_name || 'Uncategorized'}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs tabular-nums font-bold text-slate-900 dark:text-white">{format(spent)}</span>
                            <span className="text-xs text-slate-400">/ {format(total)}</span>
                            {isOver && <span className="text-[10px] font-bold text-rose-600 bg-rose-50 dark:bg-rose-950/40 px-1.5 py-0.5 rounded">Over</span>}
                          </div>
                        </div>
                        <div className="h-2.5 rounded-full bg-slate-100 dark:bg-slate-700/60 overflow-hidden">
                          <motion.div
                            className="h-full rounded-full"
                            style={{ backgroundColor: isOver ? '#ef4444' : categoryColor }}
                            initial={{ width: 0 }}
                            animate={{ width: `${pct}%` }}
                            transition={{ duration: 0.7, delay: 0.2 + idx * 0.06, ease: [0.32, 0.72, 0, 1] }}
                          />
                        </div>
                      </motion.div>
                    )
            ...
            <CardHeader className="border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-bold text-slate-900 dark:text-white">Investment Allocation</CardTitle>
                    <CardDescription className="text-sm font-medium text-slate-600 dark:text-slate-400">Portfolio breakdown by type</CardDescription>
                  </div>
                </div>
            ...
            <CardContent className="pt-6">
              {(() => {
                const INV_COLORS: Record<string, string> = {
                  stock: '#3b82f6', sip: '#8b5cf6', mutual_fund: '#6366f1', crypto: '#f59e0b',
                  fd: '#10b981', gold: '#eab308', bond: '#14b8a6', ppf: '#06b6d4', nps: '#ec4899', other: '#64748b',
                }
                const typeMap = new Map<string, { type: string; value: number }>()
                allInvestments.forEach((inv: any) => {
                  const t = (inv.investment_type || 'other').toLowerCase()
                  const val = inv.total_value || inv.quantity * inv.purchase_price
                  const existing = typeMap.get(t)
                  if (existing) existing.value += val
                  else typeMap.set(t, { type: t, value: val })
                })
                const slices = Array.from(typeMap.values())
                  .sort((a, b) => b.value - a.value)
                  .map((s) => ({ ...s, name: s.type.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()), color: INV_COLORS[s.type] || '#64748b' }))
                const totalValue = slices.reduce((s, d) => s + d.value, 0)

                if (slices.length === 0) {
                  return (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      <TrendingUp className="h-12 w-12 text-slate-300 dark:text-slate-600 mb-3" />
                      <p className="text-slate-600 dark:text-slate-400 font-medium">No investments</p>
                      <p className="text-sm text-slate-500">Add investments to see allocation</p>
                    </div>
                  )
                }
                ...
                return (
                  <>
                    <ResponsiveContainer width="100%" height={220}>
                      <RechartsPie>
                        <Pie data={slices} cx="50%" cy="50%" outerRadius={85} innerRadius={55} dataKey="value" paddingAngle={2} labelLine={false}
                          ...
                          if (active && payload?.[0]) {
                            ...
                            return (
                              <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg p-3">
                                ...
                                <p className="text-lg font-bold tabular-nums text-slate-700 dark:text-slate-200">{format(d.value)}</p>
                                <p className="text-xs text-slate-500">{totalValue > 0 ? ((d.value / totalValue) * 100).toFixed(1) : 0}% of portfolio</p>
                              </div>
                            )
                          }
                          return null
                        }} />
                        <text x="50%" y="46%" textAnchor="middle" dominantBaseline="central" className="fill-slate-400 text-[10px] font-semibold uppercase">Portfolio</text>
                        <text x="50%" y="56%" textAnchor="middle" dominantBaseline="central" className="fill-slate-900 dark:fill-white text-sm font-bold">{format(totalValue)}</text>
                      </RechartsPie>
                    </ResponsiveContainer>
                    <div className="grid grid-cols-2 gap-2 mt-4">
                      {slices.map((s) => (
                        <div key={s.type} className="flex items-center justify-between gap-2 p-2 rounded-lg bg-slate-50 dark:bg-slate-800">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: s.color }} />
                            <span className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate">{s.name}</span>
                          </div>
                          <span className="text-xs font-bold tabular-nums text-slate-900 dark:text-white flex-shrink-0">{format(s.value)}</span>
                        </div>
                      ))}
                    </div>
                  </>
